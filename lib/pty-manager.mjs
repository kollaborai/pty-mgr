...
async function handleCommand(mgr, req, daemonStartedAt, config) {
  ...
  switch (cmd) {
    ...
    case "config": {
      ...
      switch (key) {
        ...
        case "send-delay": {
          ...
          if (isNaN(ms) || ms < 0) return { ok: false, error: "value must be milliseconds (e.g. 2000)" };
          config.sendDelay = ms;
    ...
    case "send": {
      ...
      if (raw || !enter) {
        // raw mode or no enter: send as-is, no typewriter
        mgr.sendKeys(name, text);
      } else {
        // typewriter mode: send word by word to avoid paste detection
        const delay = config.sendDelay;
        const words = text.split(/(\s+)/); // keep whitespace as separate tokens
        for (const word of words) {
          if (word) mgr.sendKeys(name, word);
          if (delay > 0) await new Promise((r) => setTimeout(r, delay));
        }
        // send enter after final delay
...
usage:
  ...
  p config send-delay <ms>           delay between words (default 2000)
  ...
  p send <name> <text>               typewrite text + enter (word by word)
  p send <name> --raw <text>         send text as-is (no typewriter, no enter)
