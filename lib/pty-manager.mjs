...
async function runDemo() {
  ...
  console.log("1. spawn 'test-shell' (zsh -f in pty + xterm emulation)");
  mgr.spawn("test-shell", "zsh", ["-f"], { cols: 120, rows: 30 });
  await sleep(1000);
...
export async function runFlowWorkflow(options, deps = {}) {
  ...
  await sendFlowMessage(startAgent.session, startText, deps);
  startAgent.lastSentText = startText;
  startAgent.lastUserKey = "";
  events.push({ type: "send", from: "user", to: workflow.start.to, text: startText });
  ...
  for (let cycle = 0; cycle < maxCycles; cycle++) {
    for (const turn of workflow.turns) {
      ...
      await sendFlowMessage(toMeta.session, text, deps);
      toMeta.lastSentText = text;
      toMeta.lastUserKey = "";
      events.push({ type: "turn", cycle: cycle + 1, from: turn.from, to: turn.to, key: msg.key, text });
    }
  }
...
async function cli() {
  ...
  if (command === "flow") {
    ...
    console.error("usage: pty-mgr flow list|run");
    process.exit(1);
  }
